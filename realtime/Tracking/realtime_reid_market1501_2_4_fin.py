...
class PersonReIDWithMasks:
            ...
            if boxes is not None:
                for box in boxes:
                    ...
                    if person_img.size > 0:
                        # Extract features with masks and smart padding
    ...
    def run_camera(self):
        """Run real-time ReID with mask-based processing and smart padding"""
        cap = cv2.VideoCapture(0)
        cv2.namedWindow('Person Re-Identification with Smart Padding', cv2.WINDOW_NORMAL)
        ...
        print("- Press 'p' to show padding visualization")
        ...
        while True:
            ...
            # Add status
            status_text = f"Gallery: {len(self.gallery_ids)} | Smart Padding: ON | Masks: {'ON' if PIFPAF_AVAILABLE else 'Simple'}"
            cv2.putText(annotated_frame, status_text, (10, 30), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
            
            cv2.imshow('Person Re-Identification with Smart Padding', annotated_frame)
            ...
            if key == ord('q'):
                ...
                self._show_padding_visualization(frame)
                ...
                for box in boxes:
                    ...
                    if person_img.size > 0:
                        ...
                        if matched_id is None:
                            ...
                            print(f"Added Person {self.next_person_id} to gallery (with smart padding)")
                            self.next_person_id += 1
            ...
            if boxes is not None:
                for box in boxes:
                    ...
                    if person_img.size > 0:
                        ...
                        processed_masks = self.preprocess_masks_with_padding(masks, person_img)
                        self.visualize_masks(person_img, processed_masks)
                        break  # Show only first detection
    
    def _show_padding_visualization(self, frame):
        """Show before/after padding visualization"""
        results = self.yolo(frame, classes=0, conf=0.5)
        ...
        for r in results:
            ...
            if boxes is not None:
                for box in boxes:
                    ...
                    if person_img.size > 0:
                        # Original image
                        original = cv2.cvtColor(person_img, cv2.COLOR_BGR2RGB)
                        
                        # Smart padded image
                        padded = self.preprocess_with_padding(person_img)
                        ...
                        axes[1].imshow(padded_rgb)
                        axes[1].set_title(f'Smart Padded: {padded_rgb.shape[1]}×{padded_rgb.shape[0]}')
                        axes[1].axis('off')
    ...
    def _show_gallery(self):
        ...
        for idx, (img, person_id) in enumerate(zip(self.gallery_images, self.gallery_ids)):
            ...
            # Add label with padding and mask info
            ...
            cv2.putText(gallery_viz, "smart pad", (x1 + 5, y1 + 50),
...
def main():
    ...
    print("Enhanced PersonReID with Smart Padding and Masks")
    print("=" * 50)
    print("Features:")
    print("- Smart aspect ratio preservation (1:2 → 3:1 conversion)")
    print("- Body part mask generation")
    print("- Market-1501 compatible preprocessing")
