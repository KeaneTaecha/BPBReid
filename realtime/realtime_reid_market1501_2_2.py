...
class PersonReIDWithMasks:
    ...
    def extract_features_with_masks(self, image):
        """Extract ReID features with body part masks and proper padding"""
        
        # First apply padding preprocessing (like in extract_features)
        processed_image = self.preprocess_with_padding(image)
        
        # Generate body part masks using the ORIGINAL image (before padding)
        # This is important because pose estimation works better on original proportions
        if isinstance(image, np.ndarray):
            original_pil = Image.fromarray(cv2.cvtColor(image, cv2.COLOR_BGR2RGB))
        else:
            original_pil = image
        
        masks = self.generate_body_part_masks(original_pil)
        
        # Convert processed (padded) image to PIL
        if isinstance(processed_image, np.ndarray):
            pil_image = Image.fromarray(cv2.cvtColor(processed_image, cv2.COLOR_BGR2RGB))
        else:
            pil_image = processed_image
        
        # Apply transforms to the padded image (no resize needed since it's already 384x128)
        ...
        # Process masks: resize them to match the padded image dimensions
        # First, we need to pad the masks the same way we padded the image
        h_orig, w_orig = image.shape[:2] if isinstance(image, np.ndarray) else (image.size[1], image.size[0])
        target_ratio = 1/2
        current_ratio = w_orig/h_orig
        
        padded_masks = []
        for mask in masks:
            # Pad mask the same way as the image
            if current_ratio > target_ratio:
                # Too wide, pad height
                target_h = int(w_orig / target_ratio)
                ...
                # Too tall, pad width
                target_w = int(h_orig * target_ratio)
                pad_w = target_w - w_orig
                pad_left = pad_w // 2
                pad_right = pad_w - pad_left
                padded_mask = np.pad(mask, ((0, 0), (pad_left, pad_right)), mode='constant', constant_values=0)
            
            # Resize to final dimensions (384x128)
            mask_pil = Image.fromarray((padded_mask * 255).astype(np.uint8))
            mask_resized = mask_pil.resize((128, 384))  # (width, height)
        ...
        masks_tensor = torch.stack(padded_masks, dim=0).unsqueeze(0).to(self.device)
    ...
    def extract_features(self, image):
        ...
        try:
            ...
            # Fallback to simple feature extraction with padding
            return self._extract_features_simple_with_padding(image)
    
    def _extract_features_simple_with_padding(self, image):
        """Simple feature extraction with padding but without masks (fallback)"""
        # Use the same padding approach
        processed_image = self.preprocess_with_padding(image)
