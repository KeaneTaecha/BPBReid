...
class BatchPifPaf:
    def __init__(self, model_name: str = "shufflenetv2k16", batch_size: int = None, workers: int = None):
        ...
        self.timing_stats = {
            ...
            'num_images': 0,
            'per_image_times': [],
            'per_frame_times': []  # Track time per individual frame
        }
    ...
    def __call__(self, imagery: List[Path] or List[str], dataset_dir: List[Path] or List[str],
        ...
        with torch.no_grad():
            for batch_idx, batch in enumerate(loader):
                ...
                # Record per-frame timing (assuming batch processing)
                batch_size = len(meta_batch)
                per_frame_time = pifpaf_time / batch_size
                self.timing_stats['per_frame_times'].extend([per_frame_time] * batch_size)
                ...
                self.timing_stats['num_images'] += batch_size
                
                # Record per-batch timing
                per_image_time = batch_total_time / batch_size
                self.timing_stats['per_image_times'].extend([per_image_time] * batch_size)
...
class BatchMask:
    ...
    def __call__(self, imagery: List[Path] or List[str], dataset_dir: List[Path] or List[str],
        ...
        with torch.no_grad():
            for batch_idx, (paths, batch) in enumerate(loader):
                ...
                # Time MaskRCNN inference and filtering together
                ...
                per_frame_time = inference_time / batch_size
                self.timing_stats['per_frame_times'].extend([per_frame_time] * batch_size)
                ...
                # Update timing stats
                self.timing_stats['total_time'] += batch_total_time
                self.timing_stats['maskrcnn_inference_time'] += inference_time
                self.timing_stats['filtering_time'] += 0.0  # Combined with inference
                self.timing_stats['saving_time'] += save_time
                self.timing_stats['num_images'] += batch_size
                ...
                if verbose:
                    print(f"Batch {batch_idx + 1}/{total_batches}: "
                          f"Inference+Filtering: {inference_time:.3f}s, "
                          f"Save: {save_time:.3f}s, "
                          f"Total: {batch_total_time:.3f}s "
                          f"({per_image_time:.3f}s per image)")
    ...
    def _print_timing_stats(self, stage_name):
        ...
        print(f"Total time: {stats['total_time']:.3f}s")
        print(f"Average time per image: {stats['total_time']/stats['num_images']:.3f}s")
        print(f"MaskRCNN inference + filtering time: {stats['maskrcnn_inference_time']:.3f}s ({stats['maskrcnn_inference_time']/stats['total_time']*100:.1f}%)")
        print(f"Saving time: {stats['saving_time']:.3f}s ({stats['saving_time']/stats['total_time']*100:.1f}%)")
...
def main():
    ...
    print("\n" + "="*50)
    print("PIFPAF CONFIDENCE FIELDS GENERATION")
    print("="*50)
    ...
    print(f"\nPifPaf confidence fields saved to: {os.path.join(args.source, 'masks', 'pifpaf')}")
    print(f"MaskRCNN filtered masks saved to: {os.path.join(args.source, 'masks', 'pifpaf_maskrcnn_filtering')}")
